...
class Game {
    ...
    constructor() {
        ...
        document.querySelectorAll('.game-button').forEach((el, a) => {
            if (a < btnInfo.length) {
                this.buttons.set(el.id, new Button(btnInfo[a], el));
            }
        });
    ...
    async PressButton(button) {
        if (this.allowPlayer) {
            ...
            if(this.sequence[this.playerPlaybackPos].el.id === button.id) {
                ...
                if(this.playerPlaybackPost === this.sequence.length) {
                    this.playerPlaybackPos = 0;
                    this.addButton();
    ...
    updateScore(userName, score, scores) {
        const date = new Date().toLocaleDateString();
        const newScore = {name: userName, score: score, date: date};
        let found = false;
        for(const [a, prevScore] of scores.entries()) {
            if(score > prevScore.score) {
                scores.splice(a, 0, newScore);
                found = true;
                break;
            }
        }
